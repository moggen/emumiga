/*
lib_graphics/f_draw.c - graphics.library
	Draw function
...
int lib_graphics_f_Draw(emumsg_syscall_t *msg)
{
	...
	DEBUG(4) dprintf("graphics.library: lib_graphics_f_Draw() called\n");
	...
	if(cpu->a[1] == 0 ) {
		DEBUG(1) dprintf("Error: graphics.library: lib_graphics_f_Draw: RastPort must be provided.\n");
		return HOOK_END_PROC;
	}

	rastport = lib_graphics_s_rastport_get_vaddr(cpu->a[1]);
	if(rastport == NULL) {
		DEBUG(1) dprintf("Error: graphics.library: lib_graphics_f_Draw: RastPort pointer is invalid\n");
		return HOOK_END_PROC;
	}
...
{
	DEBUG(4) dprintf("graphics.library: lib_graphics_f_Draw_3() called\n");
