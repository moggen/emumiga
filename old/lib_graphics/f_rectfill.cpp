/*
lib_graphics/f_rectfill.c - graphics.library
	RectFill function
...
Arguments: A1.L pointer to struct RastPort
           D0.L xMin coordinate
           D1.L yMin coordinate
           D2.L xMax coordinate
           D3.L yMax coordinate
  Returns: -
****************************/
int lib_graphics_f_RectFill(emumsg_syscall_t *msg)
{
	...
	DEBUG(4) dprintf("graphics.library: lib_graphics_f_RectFill() called\n");

        cpu = msg->proc->cpu;

	DEBUG(5) dprintf("  RastPort ptr (A1): 0x%x, xMin (D0): %d, yMin (D1): %d, xMax (D2): %d, yMax (D3): %d\n",cpu->a[1],cpu->d[0],cpu->d[1],cpu->d[2],cpu->d[3]);

	if(cpu->a[1] == 0 ) {
		DEBUG(1) dprintf("Error: graphics.library: lib_graphics_f_RectFill: RastPort must be provided.\n");
		return HOOK_END_PROC;
	}

	rastport = lib_graphics_s_rastport_get_vaddr(cpu->a[1]);
	if(rastport == NULL) {
		DEBUG(1) dprintf("Error: graphics.library: lib_graphics_f_RectFill: RastPort pointer is invalid\n");
		return HOOK_END_PROC;
	}
...
int lib_graphics_f_RectFill_3(emumsg_syscall_t *msg)
{
	...
	DEBUG(4) dprintf("graphics.library: lib_graphics_f_RectFill_3() called\n");
