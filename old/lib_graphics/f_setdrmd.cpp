/*
lib_graphics/f_setdrmd.c - graphics.library
	SetDrMd function
...
int lib_graphics_f_SetDrMd(emumsg_syscall_t *msg)
{
	...
	DEBUG(4) dprintf("graphics.library: lib_graphics_f_SetDrMd() called\n");

        cpu = msg->proc->cpu;

	DEBUG(5) dprintf("  RastPort ptr (A1): 0x%x, Drawing mode (D0): %d\n",cpu->a[1],cpu->d[0]);

	if(cpu->a[1] == 0 ) {
		DEBUG(1) dprintf("Error: graphics.library: lib_graphics_f_SetDrMd: RastPort must be provided.\n");
		return HOOK_END_PROC;
	}

	rastport = lib_graphics_s_rastport_get_vaddr(cpu->a[1]);
	if(rastport == NULL) {
		DEBUG(1) dprintf("Error: graphics.library: lib_graphics_f_SetDrMd: RastPort pointer is invalid\n");
		return HOOK_END_PROC;
	}
...
int lib_graphics_f_SetDrMd_3(emumsg_syscall_t *msg)
{
	...
	DEBUG(4) dprintf("graphics.library: lib_graphics_f_SetDrMd_3() called\n");
