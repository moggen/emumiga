/*
lib_exec/f_abortio.c - exec.library AbortIO function
...
int lib_exec_f_AbortIO(emumsg_syscall_t *msg)
{
	...
	DEBUG(4) dprintf("exec.library: lib_exec_f_AbortIO() called\n");
	...
	iorequest = lib_exec_s_iorequest_get_vaddr(cpu->a[1]);
        if(iorequest == NULL) {
                DEBUG(1) dprintf("Error: exec.library: lib_exec_f_AbortIO: IORequest struct pointer is invalid\n");
...
int lib_exec_f_AbortIO_3(emumsg_syscall_t *msg __attribute__((unused)))
{
	DEBUG(4) dprintf("exec.library: lib_exec_f_AbortIO_3() called\n");
