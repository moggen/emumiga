/*
lib_dos/f_matchend.c - dos.library MatchEnd function
...
int lib_dos_f_MatchEnd(emumsg_syscall_t *msg)
{
	...
	DEBUG(4) dprintf("dos.library: lib_dos_f_MatchEnd() called\n");
	...
	entry = mmu_findEntryWithType(cpu->d[1], MMU_TYPE_DOS_S_ANCHORPATH);
        ...
        if(entry == NULL) {
                DEBUG(1) dprintf("Warning: dos.library: MatchEnd(): Attempt to use an AnchorPath that was not mapped with MatchFirst()\n");
                return HOOK_DONE;
        }
	...
	/* Delete the AnchorPath mapping and all linked AChain mappings */
	/* Important! This must be done here before the native MachEnd */
	mmu_delEntry(entry);
...
int lib_dos_f_MatchEnd_3(emumsg_syscall_t *msg __attribute__((unused)))
{
	DEBUG(4) dprintf("dos.library: lib_dos_f_MatchEnd_3() called\n");
