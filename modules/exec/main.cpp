/*
modules/exec/main.cpp - exec.library main class
...
namespace emumiga
{
   namespace exec
   {
      main::main()
      {
         DEBUG(5) dprintf("exec::main::main() called. this=%p\n", this);
      }

      int main::init()
      {
         DEBUG(5) dprintf("exec::main::init() called. this=%p\n", this);
         ...
         retc = s_ExecBase::init(this); if(retc) return retc;

         // Map the first block of memory
         zeropage_mapping = new zeropage();
         if(!zeropage_mapping)
            return ENOMEM;

         if(emu->v_space.add_mapping_absolute(zeropage_mapping, 0) != 0)
            // Failed or not at address 0!!
            return EFAULT;

         // Create an execbase special mapping
         execbase_mapping = new execbase(getID());
         if(!execbase_mapping)
            return ENOMEM;

         retc = execbase_mapping->init();
         if(retc)
            return retc;

         if(emu->v_space.add_mapping(execbase_mapping) == 0xffffffff)
            return EFAULT;

         // Extract the virtual address to the execbase object
         uint32_t execbase_address = execbase_mapping->get_base_address();

         // Create the ExecBase system object
         execbase_object = new s_ExecBase();
         execbase_object->set_address(execbase_address);

         // Let it override the proper parts of the mapping
         execbase_mapping->add_object(execbase_object);

         // Register in zeropage
         zeropage_mapping->set_execbase(execbase_address);
      ...
      void main::cleanup()
      {
         DEBUG(5) dprintf("exec::main::cleanup() called. this=%p\n", this);

         // Dispose execbase special mapping
         execbase_mapping->dispose();
         
         // Dispose zeropage
         zeropage_mapping->dispose();
      ...
      {
         DEBUG(5) dprintf("exec::main::syscall_hook_1() called. this=%p, ctx=%p\n", this, ctx);

         switch(ctx->cpu.emu_func)
         {
            case 69: return f_CloseLibrary_1(ctx);
            case 92: return f_OpenLibrary_1(ctx);
         }

         dprintf("Error: exec::main::syscall_hook_1(): Function %d is not yet emulated.\n", ctx->cpu.emu_func);
         return HOOK_ERROR;
      }

      module::hook_status main::syscall_hook_2(context *ctx)
      {
         DEBUG(5) dprintf("exec::main::syscall_hook_2() called. this=%p, ctx=%p\n", this, ctx);

         dprintf("Error: exec::main::syscall_hook_2(): Function %d is not yet emulated.\n", ctx->cpu.emu_func);
         return HOOK_ERROR;
      }

      module::hook_status main::syscall_hook_3(context *ctx)
      {
         DEBUG(5) dprintf("exec::main::syscall_hook_3() called. this=%p, ctx=%p\n", this, ctx);

         dprintf("Error: exec::main::syscall_hook_3(): Function %d is not yet emulated.\n", ctx->cpu.emu_func);
         return HOOK_ERROR;
      }

      const char *main::get_name()
      {
//         DEBUG(5) dprintf("exec::main::get_name() called. this=%p\n", this);
         return "exec.library";
      }

      module::module_type main::get_type()
      {
         DEBUG(5) dprintf("exec::main::get_type() called. this=%p\n", this);
         return MODULE_LIBRARY;
      }

      uint32_t main::get_base_address()
      {
         DEBUG(5) dprintf("exec::main::get_base_address() called. this=%p\n", this);

         if(execbase_mapping == NULL)
            return 0xffffffff;

         return execbase_mapping->get_base_address();
      }
   }
}
