/*
lib_exec/f_freevec.c - exec.library FreeVec function
...
/****************************
 Function: FreeVec
   Vector: 115
   Offset: -690
Arguments: A1.L pointer to memory or NULL
  Returns: -
****************************/
int lib_exec_f_FreeVec(emumsg_syscall_t *msg)
{
	...
	DEBUG(4) dprintf("exec.library: lib_exec_f_FreeVec() called\n");

	cpu = msg->proc->cpu;

	DEBUG(5) dprintf("  Memory ptr (A1): 0x%x\n",cpu->a[1]);
	...
	if(entry == NULL) {
		DEBUG(1) dprintf("Warning: exec.library: lib_exec_f_FreeVec(): Attempt to free non-allocated memory\n");
		return HOOK_DONE;
	}
