/*
lib_graphics/f_move.c - graphics.library
	Move function
...
int lib_graphics_f_Move(emumsg_syscall_t *msg)
{
	...
	DEBUG(4) dprintf("graphics.library: lib_graphics_f_Move() called\n");
	...
	if(cpu->a[1] == 0 ) {
		DEBUG(1) dprintf("Error: graphics.library: lib_graphics_f_Move: RastPort must be provided.\n");
		return HOOK_END_PROC;
	}

	rastport = lib_graphics_s_rastport_get_vaddr(cpu->a[1]);
	if(rastport == NULL) {
		DEBUG(1) dprintf("Error: graphics.library: lib_graphics_f_Move: RastPort pointer is invalid\n");
		return HOOK_END_PROC;
	}
...
{
	DEBUG(4) dprintf("graphics.library: lib_graphics_f_Move_3() called\n");
