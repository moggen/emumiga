/*
lib_exec/f_checkio.c - exec.library CheckIO function
...
Arguments: A1.L pointer to struct IORequest
  Returns: D0.L pointer to IORequest or NULL
****************************/
int lib_exec_f_CheckIO(emumsg_syscall_t *msg)
{
	...
	DEBUG(4) dprintf("exec.library: lib_exec_f_CheckIO() called\n");
	...
	if(iorequest == NULL) {
		DEBUG(1) dprintf("Error: exec.library: lib_exec_f_CheckIO: IORequest struct pointer is invalid\n");
...
int lib_exec_f_CheckIO_3(emumsg_syscall_t *msg)
{
	...
	DEBUG(4) dprintf("exec.library: lib_exec_f_CheckIO_3() called\n");
	...
	if(msg->arg[0]._aptr != NULL) {
		/* Returns null or the original IORequest */
		cpu->d[0] = cpu->a[1];
	}

	DEBUG(5) dprintf("  Result/IORequest ptr (D0): 0x%x\n",cpu->d[0]);
